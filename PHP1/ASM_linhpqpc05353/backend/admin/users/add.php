<?php
  ...
  $is_exist = false;
  $query = "SELECT us_email, us_phone FROM users";
  ...
  $query = "INSERT INTO users (us_name, us_email, us_phone, " . 
    "is_admin) VALUES ('$name', '$email', '$phone', $is_admin)";
  $result = mysqli_query($dbc, $query);
  mysqli_close($dbc);
    ...
    echo "
      <script>
        ...
        announceText.innerHTML = 
          'Thêm khách hàng không thành công <br/>' +
  ...
  function backToPrePage() {
    ...
    echo "
      <script>
        ...
        announceText.textContent = 'Thêm khách hàng thành công';
        backBtn.onclick = () => {
          this.location.href = 
